...
# Notice the test data it provide has a value less than 10

import math


# for most less count, we pay 1,2,4,8 for each member except the last person(highest).
# For the highest person, it could be 2^n or 2^(n-1) + 2^(n-2)
def calcMin(total_lambs):
    # 2^n - 1 = minCount 
    # minCount should less or equal to total_lambs
    doubleTop = int(math.floor(math.log(total_lambs+1, 2)))
    if doubleTop == 1:
        # in this case, we will have (1, 1)
        return 2
    FibonacciLast = 2**( doubleTop - 1 ) + 2**( doubleTop-2 )
    if FibonacciLast - 2**doubleTop + 1 <= total_lambs:
        return doubleTop + 1
    return doubleTop
...
def calcMax(remain):
    ...
    sumMin = binet(2) # F(2)
    for i in range(50):
        if remain < ( binet(i+2) - sumMin ):
            return i - 1
    raise('remain to large')

def binet(n):
    return int(math.floor(( (1+math.sqrt(5))**n - (1-math.sqrt(5))**n ) / (2**n * math.sqrt(5))))
